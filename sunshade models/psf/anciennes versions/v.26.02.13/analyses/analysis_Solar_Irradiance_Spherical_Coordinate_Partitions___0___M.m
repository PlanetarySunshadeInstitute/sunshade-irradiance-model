...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%
%	Generate unit normal vectors centered on the subsurfaces of the input unit sphere partitions to represent the Planet and the Star.  
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%	Calculate the areas of each partitioned subsurface on the Star, and the resulting flux hitting each partitioned subsurface on the Planet.               %&%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%
%	Identify the dimensions corresponding to {phi_S, theta_S} and {phi_P, theta_P} (generally {1,2} and {3,4}, but in the case of single element partitions  
%	single scalars won't register as dimensions. The subprogram below, in combination with the following if statement around the permute function, performs 
%	logical checksd to avoid these possible errors. FIX THIS: DOESN'T WORK WITH SINGLETON STAR AND MULTIPLE PLANET PARTITIONS
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%


[dimensions_S, dimensions_P] = identify_Non_Singular_Dimensions___SSSS___VV (n_phi_S, n_theta_S, n_phi_P, n_theta_P);
 

%------------------------------------------------------------------------------------------------------------------------------------------------------------------%
%	Sum solar irradiance contributions over {phi_S, theta_S}, giving total solar irradiance indexed by {phi_P, theta_P} for each Planet subsurface.      
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%


solar_irradiances_P        = sum(solar_irradiances_P, dimensions_S);


%------------------------------------------------------------------------------------------------------------------------------------------------------------------%
%	Permute the dimensions so that {phi_P, theta_P} indexing occupy the first two (while {phi_S. theta_S} disappears), then change units from km^-2 to m^-2.   
%------------------------------------------------------------------------------------------------------------------------------------------------------------------%


if sum([dimensions_P dimensions_S]) > 1 

	solar_irradiances_P    = permute(solar_irradiances_P, [dimensions_P dimensions_S]);
